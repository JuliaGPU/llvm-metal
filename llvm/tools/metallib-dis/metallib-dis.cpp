...
#include "../../lib/Target/Metal/MetalTypes.h"
...
struct metallib_program_info {
	...
	struct entry {
		...
		Optional<debug_entry> debug;
		Optional<extended_md_entry> extended_md;
		Optional<uint64_t> reflection_offset;
		Optional<reflection_entry> reflection;
	};
...
static Expected<bool> openInputFile(char** argv, std::unique_ptr<ToolOutputFile>& Out) {
	...
	Optional<uint64_t> refl_list_offset;
	Optional<uint64_t> refl_list_length;
	{
		bool found_end_tag = false;
		while (!found_end_tag) {
